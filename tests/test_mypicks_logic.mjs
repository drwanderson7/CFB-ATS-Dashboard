...
// ---------------------------------------------------------------------------
// mergeSharedPoolsIntoLocal() -- Aug 28, Drew's ask: a user should be able
// to make an unwanted admin-published pool actually stop showing up, not
// just have it silently reappear on the next sync. Real bug found: deleting
// a shared-template-originated pool only removed it from state.pools --
// mergeSharedPoolsIntoLocal()'s own guard (skip if this id is already in
// state.pools) then re-added it right back the moment sharedPools synced
// again, since nothing tracked that the deletion was a deliberate decline.
// ---------------------------------------------------------------------------
{
  const code = "function uid(){return 'x';}\n" + extractFunction("mergeSharedPoolsIntoLocal", picksSrc);

  function makeMergeCtx(pools, sharedPools, declinedSharedPools) {
    const ctx = {
      state: { pools, sharedPools, declinedSharedPools },
      saveLocal: () => {},
    };
    vm.createContext(ctx);
    vm.runInContext(code, ctx);
  ...
  {
    const ctx = makeMergeCtx([], [{ id: "sp1", name: "Kelly CFB", pickLimit: 7, games: [] }], []);
    const changed = ctx.mergeSharedPoolsIntoLocal();
    check("mergeSharedPoolsIntoLocal: adds a genuinely new shared pool", changed && ctx.state.pools.length === 1 && ctx.state.pools[0].id === "sp1");
  }
  {
    const ctx = makeMergeCtx([{ id: "sp1", name: "Kelly CFB" }], [{ id: "sp1", name: "Kelly CFB" }], []);
    const changed = ctx.mergeSharedPoolsIntoLocal();
    check("mergeSharedPoolsIntoLocal: does NOT duplicate a shared pool already present locally", !changed && ctx.state.pools.length === 1);
  }
  {
    // The actual bug: declined once, still offered by the shared catalog --
    // must NOT come back.
    const ctx = makeMergeCtx([], [{ id: "sp1", name: "Kelly CFB" }], ["sp1"]);
    const changed = ctx.mergeSharedPoolsIntoLocal();
    check("mergeSharedPoolsIntoLocal: does NOT re-add a pool this account already declined (the actual fix)", !changed && ctx.state.pools.length === 0);
  }
  {
    // A decline for ONE published pool shouldn't block a genuinely
    // different one the account has never seen before.
    const ctx = makeMergeCtx([], [{ id: "sp1", name: "Kelly CFB" }, { id: "sp2", name: "Splash pool TEST" }], ["sp1"]);
    const changed = ctx.mergeSharedPoolsIntoLocal();
    check("mergeSharedPoolsIntoLocal: a decline is scoped to that one pool id, not every shared pool", changed && ctx.state.pools.length === 1 && ctx.state.pools[0].id === "sp2");
  }
  {
    // declinedSharedPools missing entirely (an account that predates this
    // field, or normalizeState() hasn't run yet in this exact code path) --
    // must not throw.
    const ctx = makeMergeCtx([], [{ id: "sp1", name: "Kelly CFB" }], undefined);
    let threw = false;
    try { ctx.mergeSharedPoolsIntoLocal(); } catch (e) { threw = true; }
    check("mergeSharedPoolsIntoLocal: missing declinedSharedPools doesn't throw, just behaves as if nothing was ever declined", !threw && ctx.state.pools.length === 1);
  }
}
