...
// --- archivePool()/unarchivePool()/deletePoolById(): state mutation ------
{
  ...
  // deletePoolById: declining a shared/published pool (Aug 28, Drew's ask)
  {
    // makeCtx doesn't set state.sharedPools -- add it after the fact for
    // these cases specifically, since only these need it.
    const pools = [{ id: "sp1", name: "Kelly CFB" }];
    const ctx = makeCtx(pools, "overall", true);
    ctx.state.sharedPools = [{ id: "sp1", name: "Kelly CFB" }];
    await ctx.deletePoolById("sp1");
    check("deletePoolById: deleting a pool that originated from a published template records the decline",
      Array.isArray(ctx.state.declinedSharedPools) && ctx.state.declinedSharedPools.includes("sp1"));
  }
  {
    // A pool the user made themselves (never in sharedPools) shouldn't get
    // tracked at all -- nothing to decline, it was never going to reappear.
    const pools = [{ id: "p1", name: "My own pool" }];
    const ctx = makeCtx(pools, "overall", true);
    ctx.state.sharedPools = [{ id: "sp1", name: "Kelly CFB" }]; // a DIFFERENT id
    await ctx.deletePoolById("p1");
    check("deletePoolById: deleting a manually-created pool (never published) does not add anything to declinedSharedPools",
      !ctx.state.declinedSharedPools || ctx.state.declinedSharedPools.length === 0);
  }
  {
    // Deleting the same published pool twice (e.g. it got re-imported
    // somehow, or a stale double-click) shouldn't duplicate the entry.
    const pools = [{ id: "sp1", name: "Kelly CFB" }];
    const ctx = makeCtx(pools, "overall", true);
    ctx.state.sharedPools = [{ id: "sp1", name: "Kelly CFB" }];
    ctx.state.declinedSharedPools = ["sp1"]; // already declined once before
    await ctx.deletePoolById("sp1");
    check("deletePoolById: does not add a duplicate entry for an already-declined pool",
      ctx.state.declinedSharedPools.filter(id => id === "sp1").length === 1);
  }
}

// --- renderContextSelect()/renderContextSwitcherContent(): archived pools
// excluded from both dropdowns (Aug 28, real bug fix, Drew's ask) ---------
// archivePool()'s own comment always claimed archiving hid a pool from
// these two dropdowns, but neither function actually filtered on the
// archived flag -- confirmed by reading both before assuming the comment
// was accurate. This is what made "I archived it but it's still in my
// dropdown" a real, reproducible complaint rather than user error.
{
  function makeDropdownCtx(pools, activeContext) {
    const selectEl = { innerHTML: "", onchange: null };
    const viewingEl = { innerHTML: "" };
    const doc = {
      querySelectorAll: (sel) => (sel === ".ctx-select" ? [selectEl] : []),
      getElementById: (id) => (id === "ctxViewingList" ? viewingEl : { innerHTML: "", querySelectorAll: () => [] }),
    };
    const ctx = {
      state: { pools, activeContext },
      document: doc,
      esc: (s) => s,
      currentPool: () => pools.find(p => p.id === activeContext) || null,
      activeEntries: () => [],
      ctxActiveEntryId: () => null,
    };
    vm.createContext(ctx);
    vm.runInContext(extractFunction("renderContextSelect", src), ctx);
    ctx.renderContextSelect();
    return { ctx, selectEl, viewingEl };
  }

  const pools = [
    { id: "p1", name: "Active Pool" },
    { id: "p2", name: "Archived Pool", archived: true },
  ];
  const { selectEl } = makeDropdownCtx(pools, "overall");
  check("renderContextSelect: includes a non-archived pool", selectEl.innerHTML.includes("Active Pool"));
  check("renderContextSelect: excludes an archived pool (the actual fix -- this option previously still rendered)", !selectEl.innerHTML.includes("Archived Pool"));
}
{
  // renderContextSwitcherContent()'s viewRows list is inline in the
  ...
  const marker = "const viewRows=[{id:\"overall\",label:\"Overall board\"}].concat((state.pools||[]).filter(p=>!p.archived).map(p=>({id:p.id,label:p.name})));";
