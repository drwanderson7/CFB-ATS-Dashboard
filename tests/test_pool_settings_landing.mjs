...
// landing page (Sep 9, 2026). The mature pool list/import/wizard behavior
// remains underneath; the landing now uses the shared four-step pool journey.
...
check("Make picks is a primary task",html.includes('id="poolSettingsPicksBtn"')&&html.includes('>Make picks<'));
check("Track results is a primary task",html.includes('id="poolSettingsResultsBtn"')&&html.includes('>Track results<'));
check("Manage entries is retained as a secondary admin action",html.includes('class="pool-settings-admin-row"')&&html.includes('id="poolSettingsEntriesBtn"'));
...
check("weekly task copy adapts for manual-line pools",/manualLines[\s\S]*Add weekly slate & lines/.test(pools));
...
check("Make picks routes to the existing All Games board",/poolSettingsPicksBtn[\s\S]*switchTab\("board"\)/.test(init));
check("Track results routes to Results",/poolSettingsResultsBtn[\s\S]*switchTab\("record"\)/.test(init));
check("Manage entries still routes to existing My Picks subview",/poolSettingsEntriesBtn[\s\S]*switchTab\("picks"\)/.test(init));

check("landing has four-column shared onboarding styling",/\.pool-settings-task-grid\.pool-onboarding\{grid-template-columns:repeat\(4,minmax\(0,1fr\)\)/.test(css));
check("landing becomes a compact two-by-two journey on mobile",/@media\(max-width:760px\)\{[\s\S]*?\.pool-settings-task-grid\.pool-onboarding\{grid-template-columns:repeat\(2,minmax\(0,1fr\)\)/.test(css));
