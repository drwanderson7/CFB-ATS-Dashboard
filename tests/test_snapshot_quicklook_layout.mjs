...
// ---------------------------------------------------------------------------
// Snapshot stat strip on mobile: capped to the first 3 tiles (Aug 28, Drew's
// call). The horizontal-scroll design above was meant to let all 6+ tiles
// stay reachable via swipe, but a real screenshot showed the swipe
// affordance wasn't obvious -- someone seeing only the first 3 tiles with a
// hard cutoff at the card edge, no sense there was more to scroll to.
// Simplified to just showing 3, full stop, on mobile widths -- the scroll
// CSS above is deliberately left in place (now inert with only 3 tiles) so
// the check just above this one, which pins its exact presence, doesn't
// need touching for a change that's really about visibility, not scrolling.
// ---------------------------------------------------------------------------
check("mobile hides every stat tile past the 3rd (Games analyzed / Strong edges / Good edges stay, Key-number crossings/Your average pick edge/Shortlisted/pool +CLV are hidden)",
  /\.snap-strip \.snap-tile:nth-child\(n\+4\)\{display:none;\}/.test(html));
check("the mobile tile cap lives inside the same mobile media-query block as the rest of the snap-strip mobile rules, not a separate/desktop-affecting rule",
  /\.snap-tile\{flex:0 0 auto;min-width:108px;padding:2px 16px;\}[\s\S]{0,1400}\.snap-strip \.snap-tile:nth-child\(n\+4\)\{display:none;\}/.test(html));
check("desktop's own .snap-tile rule (outside the mobile media query) has no nth-child hiding -- all tiles still show on desktop",
  !/\.snap-tile\{flex:1 1 auto;padding:4px 20px;border-right:1px solid var\(--line2\);\}[\s\S]{0,200}nth-child/.test(html));
