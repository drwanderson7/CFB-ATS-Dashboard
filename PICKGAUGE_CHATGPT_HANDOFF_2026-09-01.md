...
3. **Full UI/UX review** — rendered every populated app state in a real browser (not just read from source) and found 5 real issues. Worked through 3 of them today, in priority order Drew picked:
   ...
   - **Not yet done:** the header's raw Odds API quota text (`calls left: —`) and four unlabeled icon buttons. Still open, not started.
...
## Next up
`REMAINING_TODO_2026-09-01.md` #3 (authenticated Survivor persistence acceptance) is next in the original priority order — needs Drew's real Clerk/Redis production account, not testable in a sandbox. The two remaining UI review items (#calls-left text, unlabeled icons) are smaller and can be picked up anytime.
