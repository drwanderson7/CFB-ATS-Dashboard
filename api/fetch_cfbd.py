...
GET /api/fetch_cfbd?view=advanced&year=2026
  - fetches CFBD's season advanced team stats (`/stats/season/advanced`) for
    BOTH FBS and FCS teams, with `excludeGarbageTime=true` so late-game
    score-state noise does not distort a predictive matchup comparison
  - keeps PPA/play, success rate, explosiveness, play counts, the
    standard-downs/passing-downs and rushing/passing splits, plus havoc on
    BOTH offense (havoc allowed/suffered) and defense (havoc generated)
  ...
  - LIVE-SCHEMA NOTE (Aug 30, 2026): the populated 2026 response has now
    been observed in production and the field names below match CFBD's
    current documented AdvancedSeasonStat schema, including offense.havoc,
    defense.havoc, and offense/defense plays. Access remains dict.get()-based
    so a future provider schema change still degrades to null/"—" instead of
    throwing.
...
ADVANCED_CACHE_PREFIX = "pickgauge_cfbd_advanced_v2"
...
def _advanced_splits(block):
    """One offense-or-defense split from CFBD's documented advanced schema."""
    block = block or {}
    return {
        "ppa": block.get("ppa"),
...
def _advanced_havoc(block):
    """CFBD exposes havoc on BOTH sides of the ball.

    Offense.havoc is havoc allowed/suffered by that offense; defense.havoc
    is havoc generated by that defense. Keeping both lets the client compare
    the two concepts directly inside each offense-vs-defense matchup instead
    of the old, less-useful defense-vs-defense comparison.
    """
    havoc = (block or {}).get("havoc") or {}
    return {
...
def trim_advanced_team(row, classification=None):
    """Trim one documented CFBD AdvancedSeasonStat row for Matchup Intelligence.

    The client needs descriptive season-to-date offense/defense values only;
    these NEVER feed Model #. `plays` is intentionally retained now so the UI
    can disclose how small the early-season sample really is. Garbage-time
    exclusion happens in fetch_advanced_stats(), not here.
    """
    row = row or {}
...
def fetch_advanced_stats(api_key, year):
    """Fetch predictive-context season stats for FBS plus FCS teams.

    CFBD defaults this endpoint to FBS, so the old implementation silently
    lacked matchup context for FBS-vs-FCS games. FBS remains required; FCS is
    best-effort so a transient FCS-only provider issue never takes Matchup
    Intelligence away from the primary FBS slate. `excludeGarbageTime=true`
    is deliberate for this pregame context view.
    """
    params = {"year": year, "excludeGarbageTime": "true"}
    fbs_rows = _cfbd_get(api_key, "/stats/season/advanced", {**params, "classification": "fbs"})
    combined = [trim_advanced_team(r, "fbs") for r in (fbs_rows or []) if r.get("team")]
    fcs_available = True
    try:
        fcs_rows = _cfbd_get(api_key, "/stats/season/advanced", {**params, "classification": "fcs"})
        combined.extend(trim_advanced_team(r, "fcs") for r in (fcs_rows or []) if r.get("team"))
    except (urllib.error.HTTPError, urllib.error.URLError) as exc:
        # FCS coverage is additive. Preserve the FBS panel if only the FCS
        # request fails, and tell the client via metadata so the missing side
        # can be explained rather than appearing to be a matching bug.
        fcs_available = False
        _log_server_error("advanced-stats FCS coverage unavailable", exc)
    return combined, fcs_available
...
class handler(BaseHTTPRequestHandler):
    ...
    def _handle_advanced(self, key, year, force):
        ...
        if not force and _is_fresh(cached, ADVANCED_FRESH_SECONDS):
            ...
            self._respond(200, body); return
        try:
            teams, fcs_available = fetch_advanced_stats(key, year)
        except (urllib.error.HTTPError, urllib.error.URLError):
            if cached:
                body = dict(cached); body["source"] = "stale"
                self._respond(200, body)
        ...
        payload = {
            ...
            "classifications": ["fbs", "fcs"] if fcs_available else ["fbs"],
            "fcsAvailable": fcs_available,
        }
        try:
            _kv_set(cache_key, payload)
        except Exception as exc:
