...
async function pushPoolToShared(poolId){
  ...
  try{
    ...
    if(!result.ok){
      // Test-only affordance, so still just a false return -- but log the
      // real reason instead of silently swallowing it. A 403 (someone
      // else owns this pool id) and a 409 (publish race) are genuinely
      // different problems from an expired session; worth telling apart
      // in the console even though the UI here doesn't surface it.
