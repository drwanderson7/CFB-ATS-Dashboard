...
function cpWorkflowHTML(pool,entry){
  const has=(pool.games||[]).length>0;
  if(!has)return `<div class="cp-workflow warn"><span><b>Next: import this week's sheet</b><small>That loads the exact contest slate, frozen lines, and lock time PickGauge should analyze.</small></span></div>`;
  const v=cpValidatePicks(pool,entry,true),submitted=cpSubmittedWeek(entry,pool),locked=cpIsCardLocked(pool);
  if(locked)return `<div class="cp-workflow neutral"><span><b>Card locked</b><small>This week's contest lock has passed. Review the submitted card in My Entries.</small></span><button type="button" class="btn-link-sm" data-cp-workflow-view="results">Open My Entries →</button></div>`;
  if(!v.valid)return `<div class="cp-workflow progress"><span><b>Next: build or finish the card</b><small>${esc(v.errors[0]||"Complete the required picks and confidence ranks.")}</small></span><button type="button" class="btn-link-sm" data-cp-workflow-build>Build PickGauge ranking →</button></div>`;
  if(!submitted)return `<div class="cp-workflow ready"><span><b>Card ready</b><small>All required picks and confidence values are complete. Save a dated submission snapshot before lock.</small></span><button type="button" class="btn-link-sm" data-cp-workflow-submit>Mark submitted →</button></div>`;
  return `<div class="cp-workflow ready"><span><b>Submitted ✓</b><small>Edits can still be re-submitted until lock. Your saved snapshot is available in My Entries.</small></span><button type="button" class="btn-link-sm" data-cp-workflow-view="results">View My Entries →</button></div>`;
}
